...
static const char *log_tag = "task_noti_event_groups";
#define TASK_NOTI_EVENT_GROUPS_INFO(format, ...) LOG_INFO(log_tag, format, ##__VA_ARGS__)
...
void freertos_start_task_noti_event_groups(void)
{
  //创建信号量
...
static void start_task(void *pvParameters)
{
  ...
  TASK_NOTI_EVENT_GROUPS_INFO("start_task: start");
...
//                             (TaskHandle_t *) &task3_handle );  //任务句柄
 ...
 taskEXIT_CRITICAL();
  ...
  TASK_NOTI_EVENT_GROUPS_INFO("start_task DELETE");
  //删除当前任务
  vTaskDelete(NULL);
...
static void task1(void *pvParameters)
{
  ...
  TASK_NOTI_EVENT_GROUPS_INFO("task1: running wait 4s");
  vTaskDelay(4000 / portTICK_PERIOD_MS);


  TASK_NOTI_EVENT_GROUPS_INFO("task1: taka noti event_groups give");
  res = xTaskNotify(
    task2_handle, 
    EVENTBIT_0, 
    eSetBits
  ...
  if(res == pdPASS)
  {
    TASK_NOTI_EVENT_GROUPS_INFO("task1: taka noti event_groups give success data = 0x%02x", EVENTBIT_0);
  }
  else
  {
    TASK_NOTI_EVENT_GROUPS_INFO("task1: taka noti event_groups give failed");
  }
...
{
  //任务2循环执行
  TASK_NOTI_EVENT_GROUPS_INFO("task2: running");
  BaseType_t res;
  uint8_t receive = 0;
  while (1)
  {
    res = xTaskNotifyWait(
  //当你需要接收到多个条件bit位时，就需要考虑接受前后set的bit位，其他bit位会被清理
      0x00000000,      //接收通知前是否清理原通知值，全0 表示32bit的值都是0 都不清理
      0xffffffff,      //接收到通知值后是否清理通知值，全1，表示32bit的值都清理
      &receive,
      portMAX_DELAY);

      if(receive & EVENTBIT_0)
      {
        TASK_NOTI_EVENT_GROUPS_INFO("task2: notify take reveive = 0x%02x", receive);
      }
    if(res == pdTRUE)
    {
      TASK_NOTI_EVENT_GROUPS_INFO("task2: notify take success");
    }
    else
    {
      TASK_NOTI_EVENT_GROUPS_INFO("task2: notify take failed");
    }
  }
}
...
{
  //任务3循环执行
  TASK_NOTI_EVENT_GROUPS_INFO("task3: running");
  // uint8_t res = 0;
  // char * receive = NULL;
  while (1)
  {
