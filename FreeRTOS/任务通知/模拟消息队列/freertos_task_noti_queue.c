...
static const char *log_tag = "task_noti_queue";
#define TASK_NOTI_QUEUE_INFO(format, ...) LOG_INFO(log_tag, format, ##__VA_ARGS__)
...
void freertos_start_task_noti_queue(void)
{
  //创建信号量
...
static void start_task(void *pvParameters)
{
  ...
  TASK_NOTI_QUEUE_INFO("start_task: start");
...
//                             (TaskHandle_t *) &task3_handle );  //任务句柄
 ...
 taskEXIT_CRITICAL();
  ...
  TASK_NOTI_QUEUE_INFO("start_task DELETE");
  //删除当前任务
  vTaskDelete(NULL);
...
static void task1(void *pvParameters)
{
  ...
  TASK_NOTI_QUEUE_INFO("task1: running wait 4s");
  vTaskDelay(4000 / portTICK_PERIOD_MS);

  TASK_NOTI_QUEUE_INFO("task1: taka noti queue give");
  res = xTaskNotify(task2_handle, 0x99, eSetValueWithOverwrite);
  if(res == pdPASS)
  {
    TASK_NOTI_QUEUE_INFO("task1: taka noti queue give success");
  }
  else
  {
    TASK_NOTI_QUEUE_INFO("task1: taka noti queue give failed");
  }
...
{
  //任务2循环执行
  TASK_NOTI_QUEUE_INFO("task2: running");
  BaseType_t res;
  uint8_t receive = 0;
  ...
  while (1)
  {
    ...
    if(res == pdTRUE)
    {
      TASK_NOTI_QUEUE_INFO("task2: notify take success = %d", receive);
    }
    else
    {
      TASK_NOTI_QUEUE_INFO("task2: notify take failed = %d", res);
    }
  }
}
...
{
  //任务3循环执行
  TASK_NOTI_QUEUE_INFO("task3: running");
  // uint8_t res = 0;
  // char * receive = NULL;
  while (1)
  {
