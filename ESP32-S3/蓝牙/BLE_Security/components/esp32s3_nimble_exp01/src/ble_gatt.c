         ...
         (struct ble_gatt_chr_def[]){
             {/* Heart rate characteristic */
              ...
              .flags = BLE_GATT_CHR_F_READ | BLE_GATT_CHR_F_INDICATE |  
				BLE_GATT_CHR_F_READ_ENC,   // 可读  可Indicate（带确认的通知） 和 加密读-读这个特征值要求 BLE 链路加密（配对绑定）
        ...
        .characteristics =
            (struct ble_gatt_chr_def[]){/* LED characteristic */
                                        {.uuid = &led_chr_uuid.u, // 128-bit 自定义UUID
                                         ...
                                         .flags = BLE_GATT_CHR_F_WRITE | BLE_GATT_CHR_F_WRITE_ENC,  // 只写  和 加密写-写这个特征值要求 BLE 链路加密（配对绑定）
...
int gatt_svr_subscribe_cb(struct ble_gap_event *event) {
    ...
    if (event->subscribe.conn_handle != BLE_HS_CONN_HANDLE_NONE) { //BLE_HS_CONN_HANDLE_NONE代表无有效连接句柄
