...
uint8_t cd80[5] = {0x02, 0x00, 0x80, 0x00, 0X01}; //解锁寄存器
uint8_t cd90[7] = {0x02, 0x00, 0x90, 0x00, 0X74, 0x3D, 0x0A}; // 0X70, 0x14   代码值  VREFP 参考电压输出设置   DVDD 核心电压微调   内部振荡器（OSC）频率微调  带隙基准（BG）电压微调  -- 
uint8_t cd91[5] = {0x02, 0x00, 0x91, 0x00, 0X90};
uint8_t cd89[8] = {0x02, 0x00, 0x89, 0x00, 0XF1, 0xF1, 0X00, 0x09}; //参考值 UD65640A芯片配置
uint8_t cd8A[8] = {0x02, 0x00, 0x8A, 0x00, 0x18, 0x03, 0x07, 0x04}; //参考值
uint8_t cd96[6] = {0x02, 0x00, 0x96, 0x00, 0x31, 0x00}; //参考值   设置负电源 AVEE 的输出电压和使能状态  通常需要在 ELVDD 等正电源稳定后再开启 AVEE，避免电源时序紊乱。
uint8_t cd97[6] = {0x02, 0x00, 0x97, 0x00, 0X01, 0x00}; //参考值   设置 VEMR 电源的输出电压和使能状态  必须在 ELVDD 正电源稳定后，再按顺序开启 AVEE 和 VEMR 电源，
uint8_t cd95[6] = {0x02, 0x00, 0x95, 0x00, 0x81, 0x7F}; // 0x81, 0x7F, 0x00   参考值   调整像素驱动电流   AVEE、VEMR 等负电源的稳定供电后配置

uint8_t cd51[6] = {0x02, 0x00, 0x51, 0x00, 0XFF, 0x03}; //参考值  显示亮度写入寄存器 100%

// uint8_t cd36[5] = {0x02, 0x00, 0x36, 0x00, 0X02};

uint8_t cd35[5] = {0x02, 0x00, 0x35, 0x00, 0X00}; //参考值 选择TE模式
uint8_t cd44[5] = {0x02, 0x00, 0x44, 0x00, 0X01}; //参考值  每帧在第 1 行发 1 次 TE（帧同步）
uint8_t cdA8[5] = {0x02, 0x00, 0xA8, 0x00, 0X01};  //参考值  选择 TLSB 模式
uint8_t cdAA[5] = {0x02, 0x00, 0xAA, 0x00, 0X14}; // 08  //0x01  //代码值 写入具体的 TLSB 参数值，实现低功耗扫描/时序控制  60Hz 最大 TLSB ≈ 0x15（21）
// uint8_t cdAA2[5] = {0x02, 0x00, 0xAA, 0x00, 0X01};
uint8_t cdA9[5] = {0x02, 0x00, 0xA9, 0x00, 0X01}; //参考值  TLSB参数写入使能控制
uint8_t cd11[5] = {0x02, 0x00, 0x11, 0x00, 0X00}; //参考值 退出休眠
// uint8_t cd10[5] = {0x02, 0x00, 0x10, 0x00, 0X00};
uint8_t cd29[5] = {0x02, 0x00, 0x29, 0x00, 0X00}; //参考值 打开显示
