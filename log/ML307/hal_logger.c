...
typedef struct {
    ...
    char log_buffer[LOG_LENGTH_MAX]; // 日志缓冲区
} logger_t;
...
#endif
static log_output_func_t logger_output_func = NULL;  //串口输出函数指针
...
void logger_init(log_level_t level, log_output_t output) {
    ...
    if (logger_mutex == NULL) {
        osMutexAttr_t mutex_attr = {"log_mutex", osMutexRecursive | osMutexPrioInherit, 0, 0U};
        logger_mutex = osMutexNew(&mutex_attr);
    }
#endif
...
// 注册串口日志输出函数
void logger_register_output(log_output_func_t func) {
    if (func != NULL) {
        logger_output_func = func;
    }
}
...
void logger_log(log_level_t level, const char *tag, const char *format, ...) {
    ...
    if (logger.output_mode & LOG_OUTPUT_UART) {
        if (logger_output_func != NULL) {
            logger_output_func(logger.log_buffer);
        }
    }
